...
--inscription --
DROP FUNCTION IF EXISTS creationabonne(pseudo varchar, name varchar, pnom varchar, sexe varchar, age integer, mail varchar, typeabo varchar);
...
begin
    select into res from abonne where pseudo = $1;
    if not found then
        res := false;
        return res;
    end if;
